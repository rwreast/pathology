...
def multiClass_BCE_loss(label, pred, weight=None, weight_class=None):
    ...
    _, C = label.shape
    # loss_all = 0
    loss_all = torch.tensor(0.0, device=pred.device, requires_grad=True)
...
def train_one_epoch(
    ...
    for iter, (data, label, selected) in enumerate(
        ...
        loss = multiClass_BCE_loss(
            torch.eye(num_classes)[label].to(dev), final_prob, weight_class=class_weight
        )
...
def get_parser():
    ...
    parser.add_argument(
        "--numClass", default=3, type=int, help="4:0/1/2/3, 3:12/3/4, 2:12/34"
    )
    parser.add_argument(
        "--ClassWeight",
        nargs="+",
    ...
    parser.add_argument(
        ...
        default=True,
        type=bool,
        help="threshold for 5 class classification model",
    )
...
if __name__ == "__main__":
    ...
    InternalTrain, InternalTest = get_train_ds_ZSCenter_forNuclearLevel(
