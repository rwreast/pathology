...
def show_pointcloud(pc, size=10):
    ...
    ax.scatter(pc[:, 0], pc[:, 1], pc[:, 2], s=size)  # type: ignore
...
def show_pointcloud_batch(pc, size=10):
    ...
    for i in range(B):
        ...
        ax.scatter(pc[i, :, 0], pc[i, :, 1], pc[i, :, 2], s=size)  # type: ignore
...
def show_pointcloud_2pc(pc_1, pc_2, ax=None, c1="r", c2="b", s1=1, s2=1):
    ...
    if ax is None:
        fig = plt.figure()
        ax = fig.add_subplot(1, 1, 1, projection="3d")
    ax.scatter(pc_1[:, 0], pc_1[:, 1], pc_1[:, 2], s=s1, c=c1, alpha=0.5)  # type: ignore
    ax.scatter(pc_2[:, 0], pc_2[:, 1], pc_2[:, 2], s=s2, c=c2, alpha=0.5)  # type: ignore
...
def show_pointcloud_perpointcolor(pc, size=10, c="r"):
    ...
    fig = plt.figure()
    ax = fig.add_subplot(1, 1, 1, projection="3d")
    ax0 = ax.scatter(pc[:, 0], pc[:, 1], pc[:, 2], s=size, alpha=0.5, c=c)  # type: ignore
...
def cal_auc(label, pred, pos_label=1, return_fpr_tpr=False, save_fpr_tpr=False):
    ...
    fpr, tpr, _ = metrics.roc_curve(
        label, pred, pos_label=pos_label, drop_intermediate=False
    )
    auc_score = metrics.auc(fpr, tpr)
    if save_fpr_tpr:
        if auc_score > 0.5:
            np.save(
                "./ROC_reinter/{:.0f}".format(auc_score * 10000),
                np.concatenate(
                    [np.expand_dims(fpr, axis=1), np.expand_dims(tpr, axis=1)], axis=1
                ),
            )
    if return_fpr_tpr:
